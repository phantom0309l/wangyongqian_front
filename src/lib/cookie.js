...
cookie.set = function (key, value, options) {
    if (utils.isPlainObject(key)) {
        ...
        // console.log('document.cookie', utils.encode(key) + '=' + utils.encode(value) + expires + path + domain + secure);
    }
...
cookie.all = function () {
    ...
    for (let i = 0, l = cookies.length; i < l; i++) {
        ...
        let key = utils.decode(item.shift());
        let value = utils.decode(item.join('='));
        result[key] = value;
    }
